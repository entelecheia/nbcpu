...
두 번째 LDA 모델은 기사 내의 불확실성 강도를 측정하는 데 목적이 있다. 이 모델은 불확실성과 관련된 미리 정의된 시드 단어 집합으로 초기화되어 토픽 모델링 과정을 유도한다. 각 문서의 '불확실성 점수'는 문서 내의 불확실성과 관련된 토픽의 확률 합계로 계산된다. 더 높은 불확실성 점수는 해당 문서 내에서 더 높은 수준의 정책 불확실성을 나타낸다.
