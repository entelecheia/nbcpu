...
캄보디아는 달러라이제이션(dollarization) 기반의 경제로, 독립적인 통화 정책을 시행하는 데 한계를 가지고 있다. 2000년대에 국가 통화인 리엘(Riel)의 성장과 확장에도 불구하고, 캄보디아 중앙은행(National Bank of Cambodia, NBC)은 금융 시장의 양적 및 질적 성숙도가 아직은 발전도상에 있는 상황으로 커뮤니케이션 피드백 경로가 미흡한 실정이다. 이러한 현대 통화 정책 전달 경로의 미비는 통화 정책을 관리하는 데 있어 제약 사항이 되고 있다.
...
따라서 중앙은행의 정책 불확실성을 측정하는 것은 캄보디아 경제와 금융 시장에 대한 통화정책의 영향을 이해하고 관리하는 데 중요한 과제이다. 또한 중앙은행이 대중과 금융 시장에 정책 입장을 명확하게 전달하고 투명성과 신뢰를 높이는 데에도 중요한 영향을 미친다.
...
경제 정책 불확실성에 대한 연구는 금융 시장, 경제 성과, 기업 투자, 노동 시장 역동성, 정치 양극화 등 광범위한 범위에 걸쳐 있으며 최근 연구의 중요한 주제중 하나이다. 불확실성을 측정하는 방법에는 크게 금융 데이터 기반 접근법과 텍스트 기반 접근법이 있다.
경제 정책 불확실성 측정에 대한 연구는 역동적으로 진화하고 있는 분야이다. 금융데이터 기반 측정에서 텍스트 및 주제 기반 방법까지의 다양한 접근법은 정확성, 효율성, 포괄성에 대한 지속적인 발전 방향을 보여준다. 기계 학습 기술의 통합과 글로벌 지수의 개발은 중요한 이정표를 나타낸다. 실시간, 보편적으로 적용 가능하며, 섬세한 불확실성 측정 방법에 대한 연구는 지속적인 관심의 대상이 되고 있다.

가. 금융데이터 기반 접근법

불확실성의 금융데이터 기반 측정 방식은 주로 주식 시장의 변동성과 예상치와 실제 금융 지표 간의 차이를 활용하고 있다. VIX 지수, 주식 시장의 서프라이즈, 설문 조사를 통해 수집된 예상 및 실제 거시 경제 지표 간의 차이등이 이 범주에 속하는 일반적인 방법이다(Kaveh-yazdy와 Zarifzadeh, 2021). 이러한 접근법은 불확실성을 평가하기 위한 정량적인 방법을 제공하며, 이를 직접 관찰 가능한 금융 시장 움직임과 연결하게 만들어 준다.

나. 텍스트 기반 접근법

텍스트 기반 접근법은 금융 데이터의 정량적 분석에서 통화 정책 문서, 소셜 미디어 게시물, 뉴스 기사와 같은 문서의 질적 분석으로의 전환을 나타낸다. Baker et al. (2016)이 제안한 경제 정책 불확실성(EPU) 지수는 이 분야에서 중요한 진전을 의미한다. 특정 불확실성 관련 키워드를 포함하는 기사의 빈도를 측정함으로써, EPU 지수는 다양한 분야의 정책 불확실성을 정량화했다.
그럼에도 불구하고, EPU 지수는 기사 선택의 노동 집약적 학부 학생을 통합 수작업 방식의 검증
수동 프로세스를 포함하는 문제를 가지고 있다. 이러한 제약은 프로세스를 자동화하기 위한 기계 학습 방법의 도입으로 이어졌다. Azqueta-Gavaldón (2017)는 불확실성 측정을 위해 토픽모형의 하나인 잠재 디리클레 할당(LDA)의 사용하였다. 이러한 접근법은 EPU 지수에 대한 보다 효율적인 대안을 제공했다(Azqueta-gavaldon, 2017). 후속 연구는 정확한 문서 분류와 태깅을 위해 서포트 벡터 머신과 로지스틱 회귀와 같은 정교한 기술을 더욱 도입하여 적용했다(Kaveh-yazdy와 Zarifzadeh 2021).

다. 토픽(주제) 기반 접근법

텍스트 기반 접근법 중 특히 토픽 기반 접근법은 비지도학습 방식의 특성으로 인해 다양한 연구에 확대 적용 되었다. 최근의 연구는 Larsen(2021)과 같이 사전 훈련이나 라벨링 없이 기사를 내용과 주제를 기반으로 분류하기 위해 토픽모형(LDA)을 활용하고 있다. 이 방법은 기사의 전체 단어 혼합(Mixture)이 주제 식별에 기여하는 동시에 다양한 유형의 불확실성을 구별하고 더 섬세한 이해를 제공하는 데 도움이 되었다.

비정량 데이터 기반 불확실성 측정 방법은 지속적으로 발전하고 있다. EPU 지수의 노동 집약적 특성과 제한된 적용 가능성은 추가 혁신을 촉발시켰다. 143개 국가를 포함하는 세계 불확실성 지수(WUI)와 의미 클러스터링, 단어 임베딩, 퍼지 k-평균을 사용한 새로운 접근법이 실시간 측정 및 지수 업데이트를 용이하게 하기 위해 도입되고 있다(Miranda-belmonteet 등, 2023).

1. 데이터

본 연구에 사용할 데이터는 캄보디아의 두 주요 뉴스 매체에서 기사를 수집, 필터링 및 분석하는 과정을 통해 확보한다.
...
Khmer Times는 캄보디아의 주요 영어 신문으로, 국내 및 국제 사안, 정치, 비즈니스, 라이프스타일, 문화 등 다양한 뉴스를 제공한다. 그 광범위한 경제 문제 보도로 이 연구에 귀중한 자료가 된다.

(나) Phnom Penh Post

Phnom Penh Post는 캄보디아에서 가장 오래된 독립 신문으로 알려져 있으며, 정치, 경제, 라이프스타일, 문화 등 다양한 주제에 대한 상세한 통찰력을 제공한다. 국내 경제 문제와 정책에 대한 심층 분석은 데이터셋에 중요한 가치를 부여할 것으로 예상된다.

나. 데이터 수집

데이터 수집 과정은 선택된 뉴스 매체에서 관련 기사를 수집하기 위해 설계되어 있으며, 충분한 양과 질을 확보하게 만들어 준다.

(가). 웹 스크래핑

`BeautifulSoup`과 `requests`와 같은 Python 라이브러리를 활용하여 Khmer Times와 Phnom Penh Post의 웹사이트에서 기사를 추출한다. 스크래핑 과정은 각 기사의 헤드라인, 날짜, 전체 텍스트를 포함한 필수 세부 정보를 검색한다.

(나) 키워드 필터링

연구 목표와 관련성을 유지하기 위해, 데이터셋은 "캄보디아 중앙은행," "통화 정책," "환율" 등과 같은 경제 정책과 관련된 특정 키워드를 사용하여 필터링된다. 이 단계는 중앙은행 정책 불확실성과 관련된 기사를 충분히 확보하면서도 관련 없는 데이터 수집을 최소화 하는데 목적이 있다.

(다) 기간

본 연구에서는 2014년 1월 1일부터 2023년 7월 31일까지 게시된 기사를 사용한다. 이 기간은 거의 10년에 걸친 정책 불확실성의 시계열 분석을 가능하게 한다.

나. 탐색적 데이터 분석

초기 수집은 40,002개의 기사를 산출하였으며, 전처리 후 최종 데이터셋은 39,632개의 기사로 구성되었다.

(가) 연간 분포

데이터셋의 연간 분포는 다음의 패턴을 보이고 있다. 시작연도인 2014년에는 총 2,047개의 기사가 있으며 평균 3,345단어를 가지고 있다 2015년에서 2018년 사이에는 기사의 양이 안정적으로 증가하여 2018년에 3,530개로 정점에 이르며, 평균 길이는 3,162에서 3,486단어 사이로 나타난다. 2019년에서 2020년 사이에는 기사의 양이 지속적인 확장하는 가운데 2020년에 4,688개의 기사로 정점에 이르며, 평균 길이는 약 3,400단어를 유지한다. 2021년과 2022년에는 기사가 뚜렷한 급증을 나타내어 연속 두 해에 5,493개와 6,866개의 기사가 존재한다. 평균 길이는 약 3,100단어로 약간 감소한다. 2023년에는 약 7개월 동안 3,750개의 기사가 있으며 기사는 평균 3,255단어를 가지고 있다.

(나) 전반적인 추세

데이터셋은 기사 수의 지속적인 증가를 보여주며, 캄보디아의 불확실성, 경제, 중앙은행 정책과 관련된 주제에 대한 관심이 증가하고 있음을 나타낸다. 평균 길이는 약간의 변동에도 불구하고, 전체적으로 3,247단어로 상대적으로 안정적이다.
...
중앙은행 정책 불확실성을 측정하는 것은 금리 변동, 지급준비금 요구사항 또는 기타 통화 정책 도구와 같은 중앙은행이 취한 통화 정책 결정과 관련된 불확실성을 평가하는 것을 포함한다. 이에는 몇 가지 일반적인 접근 방식이 사용된다.

1. 중앙은행 커뮤니케이션의 텍스트 분석: 이는 중앙은행이 한 커뮤니케이션, 회의록, 연설, 보고서, 보도 자료 및 기타 커뮤니케이션 형태를 분석하는 것을 포함한다. 텍스트 내용은 수동으로 분석하거나 자연어 처리 기술을 사용하여 분석할 수 있다. 연구자들은 불확실성과 관련된 키워드를 찾거나 불확실성 수준을 정량화하기 위해 토픽(주제) 모델링 또는 감정 분석과 같은 보다 정교한 기술을 사용할 수 있다. 예를 들어, Baker, Bloom, Davis EPU 지수는 이 접근법을 사용한다.

2. 금융 시장 데이터 기반 측정: 이러한 측정은 금융 시장 데이터를 사용하여 정책 불확실성 수준을 추론한다. 가정은 금융 시장이 미래 통화 정책에 대한 지각된 불확실성을 포함한 모든 가능한 정보를 통합하고 있다는 것이다. 이러한 측정의 예로는 VIX 지수와 같은 옵션 내재 변동성 측정이 있으며, 이는 시장의 미래 변동성에 대한 기대를 포착하고, MOVE 지수는 미국 국채 시장의 변동성을 측정한다.

3. 전문 예측가 설문조사: 또 다른 접근법은 전문 경제 예측가들을 대상으로 설문조사를 실시하고 그 응답을 사용하여 정책 불확실성을 측정하는 것이다. 이는 정책금리 또는 기타 통화 정책 조치의 예상 미래 경로에 대한 질문을 포함할 수 있다. 예측가들의 응답 간의 분산 또는 불일치는 불확실성의 측정치로 사용될 수 있다.

4. 계량경제학 모델: GARCH 모델(일반화된 자기회귀 조건부 이분산성)과 같은 계량경제학 모델도 정책 불확실성을 측정하는 데 사용된다. 이러한 모델은 중앙은행 정책에 영향을 받을 수 있는 경제 변수의 변동성을 추정한다. 예를 들어, 인플레이션 또는 환율과 같은 것이다.

5. 기계 학습 기술: 기술의 발전으로 기계 학습 기술은 정책 불확실성을 측정하는 데 점점 더 사용되고 있다. 예를 들어, 서포트 벡터 머신, 로지스틱 회귀 또는 신경망은 정책 불확실성과 관련된 문서를 분류하고 태그하는 데 사용될 수 있다.

나. 토픽 기반 방법의 장점

경제 불확실성을 측정하는 토픽 기반 방법은 다른 방법에 비해 여러 가지 강점을 가지고 있다.

1. 포괄성: 특정 키워드에 의존하는 방법과 달리, 토픽 기반 접근법은 기사의 모든 단어 조합을 분석하여 기사의 내용과 토픽에 대한 보다 전체적인 관점을 제공한다. 이를 통해 불확실성이 논의되는 맥락에 대한 포괄적인 이해가 가능하다.

2. 유연성: 특정 미리 정의된 키워드의 존재에 의존하지 않으므로, 때로는 거짓 긍정(False Positive, 기사가 모든 키워드를 포함하지만 실제로 측정되는 특정 유형의 불확실성과 관련이 없을 수 있음) 또는 거짓 부정(False Negative, 기사가 특정 키워드를 사용하지 않고 불확실성의 주제를 논의할 수 있음)이 발생할 수 있다.

3. 모호성 해소: 주제 기반 방법은 불확실성의 다른 유형을 구별할 수 있으며, 보다 세밀한 분석을 허용한다. 예를 들어, 기사에서 "경제 및 금융 위기", "통화 정책의 제도적 틀", "노르웨이와 EU와의 관계", "기술 및 기업 확장"과 관련된 불확실성을 구별할 수 있다.

4. 다른 측정치와의 상관관계: 토픽 기반 불확실성 측정치과 미국 VIX 및 노르웨이의 실현 주식 시장 변동성과 같은 기타 확립된 불확실성 대리인과의 긍정적인 상관관계를 발견하여, 이 방법이 유효하고 신뢰할 수 있다고 제안한다.

그러나 토픽 기반 방법에도 제한이 있다. 토픽 상관관계를 기반으로 라벨을 할당하는 과정은 주관적일 수 있으며, 측정 모형은 뉴스 미디어에서 자주 논의되지 않는 형태의 불확실성을 포착하지 못할 수 있다. 다른 방법과 마찬가지로, 토픽 기반 측정 방법은 경제 불확실성을 분석하기 위한 어러 도구 중 하나일 뿐이며, 다른 방법과 함께 사용될 때 가장 효과적일 것으로 보인다.

다. 토픽 모형

토픽 모델링은 문서 모음에서 주요 주제를 자동으로 식별하는 비지도 학습 기계 학습 기술이다. 이는 텍스트의 숨겨진 주제 패턴을 발견하고 이러한 주제를 기반으로 문서를 분류하는 방법을 제공한다.
토픽 모델링의 결과는 단어 모음으로 나타나는 토픽 목록과 그 단어에 대한 가중치 또는 확률 분포이다. 각 문서는 이러한 토픽의 혼합으로 표현된다. 아이디어는 동일한 토픽에 대한 문서에서 특정 단어 조합이 함께 나타나는 경향이 있다는 것이다. 예를 들어, 통화 정책에 관한 문서는 "이자", "금리", "중앙은행", "인플레이션"과 같은 단어를 자주 사용할 수 있다.
토픽 모델링에 사용되는 여러 알고리즘 가운데 가장 인기 있는 것 중 하나는 잠재 디리클레 할당(LDA)이다.
...
캄보디아의 고도로 달러화된 경제에서 중앙은행 정책 불확실성에 대한 연구는 금융 시장의 미발달로 인해 전통적인 통화 정책 도구가 도전을 받는 다면적인 환경을 보여준다. 본 연구는 Latent Dirichlet Allocation (LDA) 모델을 활용한 정교한 이중 모델 접근법을 사용하여 정책 불확실성을 분류하고 측정하였다. 결과는 정책 불확실성의 구체적인 원인과 시간에 따라 어떻게 변화하는지에 대한 세밀한 이해를 제공한다.
